...
void ff_jstrm_write_playlist_version(AVIOContext *out, int version)
{
    if (!out)
        return;
    avio_printf(out, "#JSTRM\n");
    avio_printf(out, "#JSTRM-VERSION:%d\n", version);
}

void ff_jstrm_write_audio_rendition(AVIOContext *out, const char *agroup,
                                    ...
                                    int name_id, int is_default, int nb_channels)
{
    ...
    avio_printf(out, "#JSTRM-MEDIA:TYPE=AUDIO,GROUP-ID=\"group_%s\"", agroup);
...
void ff_jstrm_write_subtitle_rendition(AVIOContext *out, const char *sgroup,
                                       const char *filename, const char *language,
                                       const char *sname, int name_id, int is_default)
{
    ...
    avio_printf(out, "#JSTRM-MEDIA:TYPE=SUBTITLES,GROUP-ID=\"%s\"", sgroup);
    if (sname) {
...
void ff_jstrm_write_stream_info(AVStream *st, AVIOContext *out, int bandwidth,
                                int avg_bandwidth,
                                ...
                                const char *sgroup)
{
    ...
    avio_printf(out, "#JSTRM-STREAM:BANDWIDTH=%d", bandwidth);
    if (avg_bandwidth)
...
void ff_jstrm_write_playlist_header(AVIOContext *out, int version, int allowcache,
                                    ...
                                    uint32_t playlist_type, int iframe_mode)
{
    ...
    if (allowcache == 0 || allowcache == 1) {
        avio_printf(out, "#JSTRM-CACHE:%s\n", allowcache == 0 ? "BLOCK" : "ALLOW");
    }
    avio_printf(out, "#JSTRM-TARGET:%d\n", target_duration);
    avio_printf(out, "#JSTRM-SEQUENCE:%"PRId64"\n", sequence);
    av_log(NULL, AV_LOG_VERBOSE, "JSTRM-SEQUENCE:%"PRId64"\n", sequence);

    if (playlist_type == PLAYLIST_TYPE_EVENT) {
        avio_printf(out, "#JSTRM-MODE:EVENT\n");
    } else if (playlist_type == PLAYLIST_TYPE_VOD) {
        avio_printf(out, "#JSTRM-MODE:VOD\n");
    }
    if (iframe_mode) {
        avio_printf(out, "#JSTRM-I-FRAMES\n");
    }
}

void ff_jstrm_write_init_file(AVIOContext *out, const char *filename,
                              int byterange_mode, int64_t size, int64_t pos)
{
    avio_printf(out, "#JSTRM-INIT:URI=\"%s\"", filename);
    if (byterange_mode) {
...
int ff_jstrm_write_file_entry(AVIOContext *out, int insert_discont,
                              ...
                              int iframe_mode)
{
    ...
    if (insert_discont) {
        avio_printf(out, "#JSTRM-BREAK\n");
    }
    if (round_duration)
        avio_printf(out, "#JSTRM-SEGMENT:%ld\n",  lrint(duration));
    else
        avio_printf(out, "#JSTRM-SEGMENT:%f\n", duration);
    if (byterange_mode)
        avio_printf(out, "#JSTRM-BYTERANGE:%"PRId64"@%"PRId64"\n", iframe_mode ? video_keyframe_size : size,
    ...
    if (prog_date_time) {
        ...
        avio_printf(out, "#JSTRM-TIME:%s.%03d%s\n", buf0, milli, buf1);
...
void ff_jstrm_write_end_list(AVIOContext *out)
{
    if (!out)
        return;
    avio_printf(out, "#JSTRM-END\n");
}
